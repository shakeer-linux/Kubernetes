...
```sh
$ sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --apiserver-advertise-address=192.168.33.10 --kubernetes-version stable-1.10
```
**Note:-**
...
Now if you curl the cluster IP for Jenkins service you willl still get the same respose as earlier.
...
# Challenges Faced
---
#### Issue while accessing kubectl commands
...
1. Delete Service you created for the deployment 
2. Check in ```/etc/hosts``` hostname entry for master IP. 
    
    ```sh
    <master_IP>  <master_hostname>
    e.g. - 
    192.168.33.10 master-ubuntu-xenial
    ```

3. Restart kubectl service 
4. Log out & Login again
...
After that run ```kubectl apply -f kube-flannel.yml```
...
**Refered from sources https://kubernetes.io/docs/setup/independent/create-cluster-kubeadm/ & https://blog.alexellis.io & https://stackoverflow.com/ & https://github.com/kubernetes/dashboard/wiki/Access-control**